...
use adbc_client::{
  AdbcConnectionCore as CoreConnection,
  AdbcConnectionResultIteratorCore as CoreConnectionIterator,
  AdbcDatabaseCore as CoreDatabase,
  AdbcStatementCore as CoreStatement,
  AdbcStatementIteratorCore as CoreIterator,
  ClientError,
...
impl Task for GetObjectsTask {
  ...
  fn resolve(&mut self, _env: napi::Env, output: Self::Output) -> Result<Self::JsValue> {
    Ok(_NativeAdbcConnectionResultIterator {
      inner: Some(Arc::new(Mutex::new(output))),
    })
  }
}
...
impl Task for GetInfoTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    self
      .connection
      .get_info(self.info_codes.clone())
      .map_err(to_napi_err)
  }
...
impl _NativeAdbcStatement {
  ...
  pub fn set_sql_query(&self, query: String) -> Result<()> {
    ...
    let mut stmt = mutex.lock().map_err(|e| Error::from_reason(e.to_string()))?;
...
impl Task for ExecuteQueryTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    let mut stmt = self.statement.lock().map_err(|e| Error::from_reason(e.to_string()))?;
    stmt.execute_query().map_err(to_napi_err)
  }

  fn resolve(&mut self, _env: napi::Env, output: Self::Output) -> Result<Self::JsValue> {
    Ok(_NativeAdbcStatementIterator {
      inner: Some(Arc::new(Mutex::new(output))),
    })
  }
}
...
impl Task for BindTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    let mut stmt = self.statement.lock().map_err(|e| Error::from_reason(e.to_string()))?;
    stmt.bind(std::mem::take(&mut self.data)).map_err(to_napi_err)
  }
