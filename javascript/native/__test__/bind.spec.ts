...
test.serial('statement: bind and query data', async (t) => {
...
test('statement: bind multi-batch table throws descriptive error', async (t) => {
  ...
  t.is(multiTable.batches.length, 2, 'Table should have 2 batches')

  const stmt = await conn.createStatement()
  const error = await t.throwsAsync(async () => {
    await stmt.bind(multiTable)
  })

  t.regex(error?.message ?? '', /bind\(\).*batches|batches.*bind\(\)/i, 'Error should mention bind() and batches')
  await stmt.close()
})
